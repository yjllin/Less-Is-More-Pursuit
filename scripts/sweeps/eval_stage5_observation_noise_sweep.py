"""Evaluate Stage5 robustness to LiDAR and target-observation Gaussian noise."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Stage5 observation-noise sweep for Ours, full_obs, and Euclidean."
    )
    parser.add_argument(
        "--models",
        type=str,
    ...
    parser.add_argument("--episodes", type=int, default=200, help="Episodes per noise level.")
    parser.add_argument(
        "--noise-levels",
        type=str,
        nargs="+",
        default=DEFAULT_NOISE_LEVELS,
        help="Gaussian sigma applied to normalized LiDAR and visible target-position observations.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output root. Default: runs/mappo_3d/observation_noise_sweep/<timestamp>_stage5_observation_noise.",
    )
...
def parse_noise_level(token: str) -> tuple[str, float]:
    value = float(str(token).strip())
    if value < 0.0:
        raise ValueError(f"Noise level must be non-negative, got: {token}")
    return f"{value:.2f}", value


def noise_tag(label: str) -> str:
    trimmed = label.rstrip("0").rstrip(".")
...
def run_eval(
    *,
    ...
    cmd = [
        ...
        str(out_jsonl),
        "--eval-tag",
        f"stage{stage_index + 1}_noise_{noise_label}",
        "--baseline",
        baseline,
    ]
    if deterministic:
...
def evaluate_one_model(
    *,
    ...
    for label, sigma in noise_levels:
        ...
        perception = dict(cfg.get("perception", {}))
        perception["lidar_noise_std"] = float(sigma)
        perception["target_obs_noise_std"] = float(sigma)
        cfg["perception"] = perception

        tag = noise_tag(label)
        eval_cfg_path = config_dir / f"config_stage{stage_index + 1}_noise_{tag}.json"
        eval_log_path = logs_dir / f"eval_stage{stage_index + 1}_noise_{tag}.jsonl"
        ...
        row = {
            ...
            "lidar_noise_std": float(sigma),
            "target_obs_noise_std": float(sigma),
            "stage_index": int(stage_index),
...
def main() -> None:
    args = parse_args()
    noise_levels = [parse_noise_level(v) for v in args.noise_levels]
    if not noise_levels:
        raise ValueError("--noise-levels must not be empty.")
    ...
    if args.output_dir:
        ...
        root_out = (
            ROOT / "runs" / "mappo_3d" / "observation_noise_sweep" / f"{ts}_stage5_observation_noise"
        ).resolve()
    ...
    if all_rows:
        merged_jsonl = root_out / "full_observation_noise_sweep_summary.jsonl"
        with merged_jsonl.open("w", encoding="utf-8") as fp:
            for row in all_rows:
                fp.write(json.dumps(row, ensure_ascii=False) + "\n")

        merged_csv = root_out / "full_observation_noise_sweep_summary.csv"
        ...
        print("[Warn] observation-noise sweep produced no evaluation rows.")
